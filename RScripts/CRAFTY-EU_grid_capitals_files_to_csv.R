...
if (doPA_EU) { 
    ...
    setwd("~/Nextcloud/workspace_newEU/CRAFTY-EU input files/")
    
    PA200_shp  = readOGR("PAs/PA data/PA200_ext_LINKS.shp") 
    ...
    PA200_r = raster(paste0(path_output, "/PA200_PA.tif"))
}
...
smoothCapitals <- function(r_in, boundary_r, width_m=5000, maskchar=NA) { 
    ...
    r_out <- focal(r_out, w = win, fun = mean, pad = TRUE, na.rm=T)
...
if (do4Capitals) { 
    ...
    for (c_idx in 1:length(four_capital_names)) { 
        ...
        foreach (layer_idx = 1:length(layer_names), .packages = c("raster")) %dopar% {
            ...
            BNG_r_tmp4 = smoothCapitals(BNG_r_tmp3, boundary_r = CHESS_mask_r, maskchar=NA, width_m = 5000)
            plot(BNG_r_tmp4)
            # plot(BNG_r_tmp4 - BNG_r_tmp2)
